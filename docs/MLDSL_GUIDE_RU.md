...
- Аргументы: конвертер читает `chestItems` (снимок сундука параметров) и пытается восстановить значения по слотам/типам предметов (книга=текст, slime_ball=число, magma_cream=переменная, item_frame=массив, paper=место) + enum’ы по `●/○`.
- Если данные не найдены/не распознаны, конвертер **не молчит**: добавляет `# WARN:` и оставляет вызов без части аргументов.
